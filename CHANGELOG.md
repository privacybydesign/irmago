...
## Unreleased
### Added
- IETF Token Status List support for mso_mdoc credentials, closing the gap where revocation only worked for SD-JWT VCs ([#725](https://github.com/privacybydesign/irmago/issues/725)). An mdoc's MSO can now carry a `status.status_list` reference (draft-ietf-oauth-status-list-15 §6.3.2), read into the same `statuslist.Reference`/`StatusClaim` types the SD-JWT VC path already uses. Checking that reference requires the CWT/COSE encoding of the Status List Token itself (§5.2), which `eudi/credentials/statuslist` did not support before — JWT and CWT are now dispatched on automatically, by sniffing the fetched bytes (a JWT is ASCII; a CWT is CBOR), so `statuslist.Checker` handles either without its callers caring which one an issuer publishes. CWT signature verification is x5chain-only, mirroring both this codebase's own mdoc issuance and the JWT path's x5c convention (trust from the configured X509VerificationContext, not from whatever chain the token carries); the JWT path's kid+did:web/did:jwk resolution has no established equivalent in ecosystems that use CWT/COSE and stays out of scope. `mdoc.Verifier.SetStatusChecker` wires a checker into mdoc verification, enforcing the same fail-closed policy `sdjwtvc`'s holder verifier already applies (anything but a `Valid` bit refuses the credential), and `mdoc_credential_service` now persists each document's status reference, seeds `LastKnownStatus` at issuance, and reports `Revoked`/`RevocationSupported` in the credential list — previously hardcoded to false for every mdoc. **Breaking (internal API):** `services.NewRevocationService` takes a variadic `...db.CredentialStatusStore` instead of a single `db.SdJwtVcStore`, so one service's background sweep and credential-list flags now cover every format that carries a status reference; `services.NewMdocCredentialService` takes a `*RevocationService`; `db.MdocStore` gains `ListInstancesWithStatusReference`, `ListStatusReferencedInstanceStatuses` and `UpdateInstanceStatus` (via the new shared `db.CredentialStatusStore` interface), and `models.MdocBatchInstance` gains the matching `StatusListURI`/`StatusListIdx`/`LastKnownStatus`/`LastStatusCheckAt` columns. An mdoc disclosure's permission screen and activity log entry now show `Revoked`/`RevocationSupported` too, as they already did for SD-JWT VCs: `mdoc_dcql.NewMdocDcqlHandler` takes a `mdoc_dcql.RevocationChecker` (pass the wallet's `*services.RevocationService`, or nil). `services.RevocationService.IsRevoked` now takes the `*statuslist.Reference` (read off an instance with the new `StatusReference()` on both instance models) instead of an SD-JWT VC instance, and `BatchRevocation` takes the store whose batches to report. The COSE_Sign1 handling mdoc and the CWT Status List Token share (algorithm allow-list, decoder, x5chain parsing) lives in the new `eudi/credentials/coseutil`, and the fail-closed check both holder verifiers apply is `statuslist.Checker.RequireValid`.

### Changed
- **Breaking (internal API):** the JWT-only names in `eudi/credentials/statuslist` now say JWT, now that CWT siblings exist: `StatusListTokenTyp` → `StatusListTokenJWTTyp`, `StatusListTokenContentType` → `StatusListTokenJWTContentType`, and in the test helpers `SignToken` → `SignJWTToken`, `SignTokenWithTyp` → `SignJWTTokenWithTyp`, `TestStatusListServer.Serve` → `ServeJWT`. `models.StatusListCacheEntry.RawJwt` is now `RawToken`; its database column stays `raw_jwt`.
