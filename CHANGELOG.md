...
- `CredentialType.RandomBlindAttributeNames`, which tells the client which random blind attributes an issuance credential request expects, returned identifiers that were off by one: the underlying index helper counts the metadata attribute at position 0, so matching those indices against the 0-based attribute slice reported the wrong identifier (or none). The method now reads each attribute's `RandomBlind` flag directly. The fix is gated on protocol version 2.9 so it is not a breaking change: the corrected identifiers and the strict pre-session consistency check activate only once both peers negotiate 2.9, while peers below 2.9 keep exchanging the historical identifiers via `RandomBlindAttributeNamesLegacy` and interoperate exactly as before. (Protocol 2.9 is not yet enabled server-side, so this currently corrects server-side logging and lands the machinery; the wire behaviour switches over when 2.9 ships.) Alternative to the breaking fix in [#151](https://github.com/privacybydesign/irmago/pull/151).
