...
- Support for the IETF OAuth Token Status List (draft-ietf-oauth-status-list-15) on SD-JWT VC credentials
  ...
  - Status List Token signatures are validated via the same `x5c`-or-`kid`+DID (did:web/did:jwk) dispatcher as the SD-JWT VC itself, refactored into the shared `eudi/jwt.JwtKeyProvider`. The token is bound to the credential via its `sub` claim (which must equal the referenced `uri`) plus a trusted signature; a delegated Status Issuer signing with its own key is supported, per draft-15 §11.3. A stricter `iss`(token) == `iss`(credential) binding is available opt-in via the `RequireStatusListIssuerMatch` flag
  - Status checks run at every credential lifecycle site: holder-side at issuance (fail-closed), verifier-side after SD-JWT VC verification, and a background `StatusRefreshService` keeps stored credentials' `LastKnownStatus` up to date. At disclosure the wallet does not fail closed — a revoked instance is surfaced on the OpenID4VP disclosure plan with `Revoked=true` (matching IRMA), leaving the decision to the frontend and the verifier's own status check
