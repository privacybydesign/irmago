...
## Unreleased
### Added
- Integration tests for the full mso_mdoc flow against real services: OpenID4VCI issuance of the EU Age Verification mdoc (`eu.europa.ec.av.1`) from the EUDI reference Python issuer, and OpenID4VP disclosure of it over plain `direct_post` to the EUDI verifier container — no existing test pushed an mdoc through either an issuance flow (the previous mdoc test seeds the credential store directly) or an unencrypted response mode. The issuer's Age Verification configuration is enabled in the test config with a dedicated `countries.AV` section, because the issuer hardcodes country `"AV"` for that scope and signs with that country's key; recreate `eudi_pid_issuer_py` after pulling. **These tests currently fail, deliberately**: they surfaced that the wallet's mso_mdoc issuance parser expects a full MDoc document (`{docType, issuerSigned}`) where OpenID4VCI's ISO mdoc credential format specifies — and the reference issuer emits — the base64url-encoded CBOR `IssuerSigned` structure (`{nameSpaces, issuerAuth}`), so every spec-compliant mdoc issuance fails with `decode cose: empty COSE_Sign1`. The tests are kept as the executable specification of the fix; the finding and its evidence live in `docs/plans/eudi-pid-python-mdoc-av-tests.md`.
...
### Fixed
- `testdata/eudi/verifier/keystore.p12` was truncated by git's text normalization — `.gitattributes` declares `* text eol=lf` and nothing marked `.p12` as binary, so committing the regenerated keystore stripped a CR byte out of the PKCS12 blob. Both EUDI verifier containers crash-looped on startup with an `EOFException` while loading it. The keystore is rebuilt from the committed key and certificates (no re-signing, same authorized set), and `.p12`, `.der` and `.crl` are now marked binary so it cannot happen again.
