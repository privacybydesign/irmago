...
## [1.2.0] - 2026-07-22
### Added
- `clientmodels.Attribute` now carries all values the verifier accepts for an attribute (`requested_values`, optional) instead of only the first: OpenID4VP DCQL (section 6) allows several acceptable values per claim, of which the holder must disclose one. Populated on OpenID4VP DCQL obtainable and unobtainable descriptors and on classic-IRMA issuance-during-disclosure bundles, so frontends can lock their obtain flows to a choice between the accepted values (privacybydesign/irmamobile#583). `requested_value` keeps duplicating the first accepted value for backwards compatibility.
