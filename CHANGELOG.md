...
### Changed
- The single-use mdoc instance logic that OpenID4VP disclosure carried inline is now shared rather than duplicated, so proximity and OpenID4VP cannot drift apart on it: `services.MdocInstanceSelector` owns choosing an unused instance and spending it (including the batch-of-one exemption, and the requirement that spending happens only after everything that can fail already has), `services.SelectiveDiscloseNamespaces` and `services.RevealFromClaimPaths` own stripping a document to the selected elements, and `dcql.SelectionsFromChoices` owns mapping a user's disclosure choice onto the DCQL query it answers. The last of those moved out of package `client`, which keeps a delegating wrapper so its call sites are unchanged. Behaviour is unchanged on the OpenID4VP path; an unlinkability property that held on one transport and not the other would look like nothing being broken.
