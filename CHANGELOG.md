...
### Fixed
- `did:key` DID URLs carrying a verification-method fragment (`did:key:z6Mk...#z6Mk...`), the form the did:key specification recommends for referencing the key, now resolve: the fragment is stripped before decoding, as the did:jwk resolver already did. Previously such a `cnf.kid` failed base58 decoding ([#576](https://github.com/privacybydesign/irmago/issues/576)).
