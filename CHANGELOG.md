...
- The recognized-list channel of the trust ladder: a party granted on a trust list the wallet recognizes reports `trust_level: "medium"`. The list is a JSON list of trusted entities in the shape of ETSI TS 119 602, signed as a compact JAdES-B-B, with role-typed entries — trust as an issuer is not trust as a verifier — matched against a party's certificate, that certificate's subject key identifier, its organization identifier, or an identifier outside X.509 such as a DID. The wallet keeps its own copy, which counts only while its signature verifies against the anchors pinned for that list, the current time is before its `nextUpdate`, and its sequence number has not regressed. Every way that can fail — an unreachable, tampered, expired or rolled-back list — is absent evidence rather than an error: the party is ranked by the certificate channel alone and the session runs on. The two channels are independent, so the rung is the stronger of the two. Each session pins one snapshot of the lists, so a refresh landing halfway through cannot change what that session already decided. No list is recognized yet — Yivi's own is not published — so the channel contributes nothing until one is compiled in, and `client.WithRecognizedLists` points a test wallet at a list of its own ([#657](https://github.com/privacybydesign/irmago/issues/657)).

### Changed
- **Breaking:** `clientmodels.TrustedParty` carries a `TrustLevel` (`"low"`, `"medium"`, `"high"`) instead of the boolean `Verified`, and the app must update in lockstep: derive whatever it showed for `verified` from `trust_level` being medium or high. The level says how strongly a party is vouched for, never whether it is who it claims to be: identity validation is a separate gate that fails the session, and a low level never blocks one. The field is absent when nothing was evaluated, which is not the same as low. What the wallet reports today: an EUDI verifier whose certificate chain validates against the Yivi trust anchors and an IRMA issuer or requestor registered in a valid scheme rank high, everyone else ranks low. Medium arrives with the recognized trust list, which no list is compiled in for yet ([#655](https://github.com/privacybydesign/irmago/issues/655)).
