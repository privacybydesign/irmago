...
- The storage regression tests now check that stored data can still be used, not only read. They disclose the stored IRMA credentials and the stored OpenID4VCI SD-JWT before issuing anything fresh, and require them to be the only candidate. Before, a freshly issued copy could be disclosed instead, so broken stored keys went unnoticed. Each wallet is also closed and opened a second time before the checks, so a migration that only works in memory fails. A missing snapshot now fails its test instead of skipping it, and `TestEveryStorageSnapshotHasATest` fails when a snapshot folder has no test.
- Storage snapshots no longer expire. IRMA credentials default to 6 months' validity, and an IRMA-issued SD-JWT carries the same expiry as `exp`, which an OpenID4VP verifier enforces. The v0.19.2 snapshot would have started failing on 2026-09-30, and the others by March 2027. The generator now issues IRMA credentials valid for 20 years. Stored IRMA disclosures skip the server's expiry check. For the snapshots up to v1.4.0, whose email SD-JWTs will still expire, the test discloses a freshly issued one once the stored one has expired.
- The v1.4.0 storage regression test now checks the stored mdoc's device keys. Each of its 30 instances signs with its stored key, and the signature is verified against the device key in its MSO, with the verifier's clock inside the MSO's validity. The stored mdoc could not be disclosed for this, because its MSO expires 90 days after the snapshot was made, so the device key table went unchecked before.
