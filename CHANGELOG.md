...
- Terminal session states are no longer dispatched twice: `session.finish` uses its eviction from the session manager as the guard, so a protocol's own report and `HandleUserInteraction`'s dismissal backstop can no longer both emit an event.
