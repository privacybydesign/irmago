...
- A session that ends because the party on the other end failed identity validation now reports `error_type: "party_validation_failed"` on `clientmodels.SessionError`, set by both EUDI protocol paths: a verifier whose certificate chain, request signature or DID does not hold up, and an issuer whose credential does not verify against its own chain. The app can switch on it to say the request was not trustworthy and that nothing was shared, instead of showing the generic error screen a network or protocol failure gets. Those keep reporting no error type ([#656](https://github.com/privacybydesign/irmago/issues/656)).
- The recognized-list channel of the trust ladder: a party granted on a trust list the wallet recognizes now ranks `medium`. A list is an ETSI TS 119 602 List of Trusted Entities in JSON, signed as a compact JAdES-B-B whose `x5c` chain validates against the pinned Yivi anchors, with role-typed entries — trust as an issuer is not trust as a verifier — keyed on certificate or subject key identifier plus the entity's `organizationIdentifier`, and on the bare DID for entries carrying one under the `other_ids` type `did`. `Client.RefreshTrustLists(ctx) error` downloads the lists and adopts the ones that hold up; a list counts only while its signature verifies, it is before its `next_update`, and its sequence number has not gone backwards. Everything about the channel is fail-soft: an unreachable, tampered, expired or replayed list is absent evidence, so parties cap at `low` and sessions run on. The certificate and list channels are independent and a party lands on the higher rung, so a scheme-certified verifier stays `high` with every list down. Sessions never fetch — each pins one snapshot of the lists, so a refresh landing halfway through cannot change what that session decided ([#657](https://github.com/privacybydesign/irmago/issues/657)).
- `lote.RecognizedSources`: the compiled-in set of recognized lists a released wallet consults. It is empty until Yivi publishes its list, so a released wallet ranks parties by the certificate channel alone, exactly as before. A wallet that must consult a different set — an integration test serving a list it controls, or a staging build — names it in `client.Config.RecognizedTrustLists`.
- The top rung through the recognized-list channel: an entry on Yivi's own list carrying the `onboarded-by-yivi` marking now ranks `high`, the same rung a Yivi scheme certificate reaches, because both are Yivi vouching for the party. Yivi publishing a list is not Yivi vouching for everyone on it, so an unmarked entry stays `medium`; the same marking on a list Yivi does not operate is that operator's claim about Yivi and stays `medium` too. The channels remain independent, so a scheme-certified party is still `high` with every list down ([#659](https://github.com/privacybydesign/irmago/issues/659)).
- The certificate channel of the trust ladder now reaches OpenID4VCI issuers: an issuer that signs its credentials with an `x5c` chain validating against the wallet's trust anchors ranks `high`, the same rung a certificate-authenticated verifier reaches. `sdjwtvc.VerifiedSdJwtVc` carries the validated end-entity certificate (`IssuerCertificate`, nil for a DID-identified issuer) that the holder verification path previously validated and dropped. Each offered credential is ranked separately, off the certificate and `iss` it was actually signed with, alongside the session's own issuer rung — and both are ranked after the credentials are fetched, so the permission screen and the issuance log show what was verified rather than what the issuer's metadata claimed at offer time. A `low` issuer still issues: the rung is rendered, never enforced ([#660](https://github.com/privacybydesign/irmago/issues/660)).
...
### Changed
- **Breaking:** `clientmodels.TrustedParty` carries a `TrustLevel` (`"low"`, `"medium"`, `"high"`) instead of the boolean `Verified`, and the app must update in lockstep: derive whatever it showed for `verified` from `trust_level` being medium or high. The level says how strongly a party is vouched for, never whether it is who it claims to be: identity validation is a separate gate that fails the session, and a low level never blocks one. The field is absent when nothing was evaluated, which is not the same as low. What the wallet reports today: an EUDI verifier whose certificate chain validates against the Yivi trust anchors and an IRMA issuer or requestor registered in a valid scheme rank high, a party granted on a recognized trust list ranks medium, everyone else ranks low ([#655](https://github.com/privacybydesign/irmago/issues/655)).
- **Breaking:** what an EUDI party is called and pictured now resolves by how strongly it is curated, through one helper both protocol paths compose through: the name on the recognized-list entry first, the name its validated certificate carries second, and the name it gives itself last. A logo only ever comes from a source beyond the party — the list entry or the certificate — so a party nobody vouches for is shown with its own name, its identifier, and no logo. For the app this means the issuer party on an OpenID4VCI permission request now arrives without an `Image`: nothing vouches for an issuer until its certificate reaches the trust ladder, and the logo in its own credential-issuer metadata is the issuer's own word. The logo on a *credential's* issuer, which the credential list and the activity log render, is unchanged. Curated logos are downloaded on first sight and cached like any other, keyed by their URI ([#659](https://github.com/privacybydesign/irmago/issues/659)).
- **Breaking:** `client.New` takes a single `client.Config` instead of eight positional arguments, and is the wallet's only constructor. Fields are named because three of them are plain string paths (`StoragePath`, `IrmaConfigurationPath`, `EudiAppDataPath`) that a caller could transpose without the compiler noticing; the zero value of a field takes a documented default, so `Locale: ""` is the fallback language and `RecognizedTrustLists: nil` is the compiled-in set. It also replaces `NewWithRecognizedTrustLists`, which existed only to vary that last field.
