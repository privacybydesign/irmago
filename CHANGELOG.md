...
- mDoc (ISO 18013-5, `mso_mdoc`) credentials are wired into the real issuance and presentation paths, replacing the standalone prototype wire format added in 1.1.1. Issuance runs through the generic OpenID4VCI client via a new format-parser seam (`services.CredentialFormatParser`, registered per format in `client.New`), so credential offer, token request, nonce endpoint and proof of possession are shared with SD-JWT VC rather than duplicated; presentation runs through a `mdoc_dcql` DCQL handler alongside the existing SD-JWT handlers. `mso_mdoc` is accepted by the credential-configuration validators, and the verifier's `response_uri` is threaded through `PrepareDisclosure` because mdoc's `deviceAuth` signs over the OpenID4VP session transcript, whose handover info is `[clientId, nonce, null, responseUri]` — the CBOR null holds the place of the response encryption JWK thumbprint, which this wallet does not use; SD-JWT's key-binding JWT ignores it.
