...
## Unreleased
### Changed
- Update `github.com/privacybydesign/gabi` from the December 2022 pseudo-version `68a0869` to `1d483a2` (2026-07-30), 67 commits later. No existing gabi API changed (the bump only adds `gabi.NewProofRandomizers`), and nothing on the wire changed, so an updated irmago still issues to and discloses against older peers. The bump pulls in the `signed`, `gabikeys`, `revocation` and `credential` fixes listed under Fixed below. gabi's own `go` directive is 1.26.4, lower than irmago's 1.27, so it does not move irmago's minimum Go version. Carried along: `github.com/mr-tron/base58` 1.1.3 to 1.3.0 (used by `eudi/did`), `github.com/minio/sha256-simd` 0.1.1 to 1.0.1, `github.com/multiformats/go-multihash` 0.0.11 to 0.2.3, and the indirect deps that follow from those ([#711](https://github.com/privacybydesign/irmago/issues/711))

### Fixed
- Scheme index signature verification no longer panics on an unparseable signature. `signed.Verify` indexed the decoded ASN.1 integers without checking there were two of them, so an `index.sig` holding a SEQUENCE of fewer than two integers panicked instead of returning an error. `Configuration.verifySignature` recovers from that panic, but `checkRemoteTimestamp` — which verifies the index signature a scheme server just served, before any update is applied — does not, so the panic escaped `InstallScheme`, `UpdateScheme` and `UpdateSchemes` and reached whatever called them. A signature with trailing bytes after the SEQUENCE is now rejected as well. Fixed upstream in [gabi#73](https://github.com/privacybydesign/gabi/pull/73)
- Parsing a scheme public key that is not PEM no longer panics. `signed.UnmarshalPemPublicKey` dereferenced the `*pem.Block` that `pem.Decode` returns, and that is `nil` for input holding no PEM block, so a scheme whose `pk.pem` is not PEM panicked on the same signature-check path; `yivi sign` did the same for a non-PEM private key. Both now return an error. Fixed upstream in [gabi#57](https://github.com/privacybydesign/gabi/pull/57)
- An issuer public key whose `<Bases num="...">` disagrees with the number of `<Base_i>` elements it holds, or whose bases are not base 10 integers, is now rejected by `gabikeys.NewPublicKeyFromBytes`, `NewPublicKeyFromFile` and `NewPublicKeyFromXML`. Previously the unmarshaler sized the base slice from `num` and then indexed the elements: a `num` above the element count panicked with an index out of range, a `num` below it dropped the surplus bases without a word, and a base that is not a base 10 integer became a `nil` entry that only surfaced downstream. `irma.Configuration` parses these keys out of a scheme directory, so all three cases were reachable from scheme data
- Writing an issuer private key over an existing file now pins the file mode to 0600. `gabikeys.PrivateKey.WriteToFile` used `os.Create` on the overwrite path, which leaves the mode of an existing file alone and creates a new one 0666 minus umask, so `yivi scheme issuer keygen -f` could leave the private key it overwrote world-readable
- Concurrent disclosures of the same credential no longer race on its non-revocation witness. `revocation.NewProofCommit` wrote the per-proof randomizer into the `Witness`, and `IrmaClient.credentialsCache` hands every session the same `*gabi.Credential`, so two sessions disclosing one credential at once wrote to the same `Witness`. `NewProofCommit` now works on a copy
- A non-revocation `Commit` failure during disclosure returns an error instead of panicking
