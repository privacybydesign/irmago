...
- `client.NewWithRecognizedTrustLists`: `client.New` with the compiled-in recognized-list set replaced, for integration tests and staging builds. `New` is unchanged and passes `lote.ProductionSources`, which is empty until Yivi publishes its list — so a released wallet ranks parties by the certificate channel alone, exactly as before.
