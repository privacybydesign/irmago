...
- An X.509 party whose chain the wallet cannot trace to any anchor no longer fails the session: a chain that ends nowhere the wallet knows proves nothing, so its holder passes the identity gate as a legitimate-looking stranger and ranks `low`, with the certificate's contents — name included — treated as the party's own word. The same goes for a revoked certificate: revocation withdraws the vouching rather than breaking the request. An entry on a recognized trust list keyed on that very certificate still lifts the party, all the way to `high` on Yivi's own list. The gate keeps rejecting genuinely broken requests — a bad signature, a client_id the certificate does not cover, a certificate outside its own validity window — with `party_validation_failed`.
