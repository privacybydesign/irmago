...
## Unreleased
### Added
- The `org-iso-mdoc` protocol of the W3C Digital Credentials API is modelled in `eudi/credentials/mdoc/dcapi.go`: `DCAPIEncryptionInfo` (the reader's nonce and the COSE recipient key it wants the response sealed to) and `DCAPIEncryptedResponse` (the HPKE encapsulated key and the sealed `DeviceResponse`), both behind the shared `["dcapi", …]` envelope decoder that refuses another protocol's message by name rather than by a missing map key. ISO/IEC 18013-7 is paywalled, so both shapes are pinned byte-for-byte against a captured Age Verification exchange — including rebuilding `EncryptionInfo` from an `*ecdsa.PublicKey` and landing on the reader's exact 125 bytes, which checks our COSE_Key encoding against a real implementation rather than against our own decoder. The recipient key is kept as `cbor.RawMessage` and the encoder never used to rebuild a received structure: the session transcript hashes the base64url of the bytes **as they arrived**, so re-encoding a non-canonical reader's map would move the digest and fail `deviceAuth` with an error pointing at the signature instead of the encoder. `mdoc.NewDCAPISessionTranscript` builds the handover that binds it — `[null, null, ["dcapi", SHA-256(cbor([base64url(EncryptionInfo), origin]))]]`, taking the encoded string rather than the structure for the same reason. It sits beside the QR and NFC handovers rather than beside the OpenID4VP DC API one, because it is ISO 18013-7 and not OpenID4VP; the two share a transport and are not interchangeable. `mdoc.SealDCAPIResponse`/`OpenDCAPIResponse` complete the cryptography, over `DHKEM_P256_HKDF_SHA256 / HKDF_SHA256 / AES_128_GCM` with `info` the encoded session transcript and an empty `aad`, on Go 1.27's stdlib `crypto/hpke` — no new dependency and no cgo. `openid4vp.DcApiProtocolIsoMdoc` names the protocol and refuses it deliberately rather than as an unknown one: it cannot be another branch of `parseDcApiRequest`, since that returns an OpenID4VP `AuthorizationRequest` and an org-iso-mdoc request carries a `DeviceRequest` with no `client_id`, `nonce`, DCQL or `response_mode`. Still to come: the session that drives these, and an `origin`, which reaches a wallet from the browser or OS and which irmamobile does not yet supply.
- The ISO/IEC 18013-5 request structures both transports need: `DeviceRequest`/`DocRequest`/`ItemsRequest` (8.3.2.1.2.1) with `ItemsRequestBytes` preserved exactly as received because 9.1.4 signs them, the `SessionTranscript` type with its `QRHandover`/`NFCHandover` constructors (9.1.5.1), Table 8's response status codes and `documentErrors` (8.3.2.1.2.2/8.3.2.1.2.3), and `deviceMac` (9.1.3.5) — verified as a reader, never produced, with `devicemac_policy_test.go` parsing every non-test file to keep it that way. `eudi/isomdoc.DcqlQueryFromDeviceRequest` translates such a request onto the DCQL pipeline OpenID4VP already uses. Device retrieval — engagement, the session channel and BLE framing — also consumes these structures, but that work is parked and nothing here depends on it.
- `eudi/isomdoc` runs an ISO/IEC 18013-5 presentation end to end: `Session.Respond` takes an `org-iso-mdoc` request delivered by the W3C Digital Credentials API, builds the session transcript, authenticates the reader **per document** (7.2.1 forbids a session-level refusal, since an mDL must release its mandatory elements to an unauthenticated reader and an age-verification credential must release nothing), asks the wallet, signs `deviceAuth` over the transcript and returns the HPKE-sealed `DeviceResponse`. The wallet is behind a `Discloser` interface, so candidate selection, consent and single-use instance accounting stay where OpenID4VP already has them rather than being reimplemented. Ordering is the load-bearing part: the transcript exists before anything is verified against it, and the wallet commits only once the response is sealed — spending a single-use instance on a response that was never produced is the correlation that batch issuance exists to prevent. The package was `eudi/proximity`; it is named for the presentation now that the transport is not BLE.
...
- **Breaking (internal API):** removed `db.CredentialStore.GetBatchesByVCT` (now `db.SdJwtVcStore`), which had no callers. It matched on `verifiable_credential_type` alone with no format guard — harmless today, since mso_mdoc has its own table, store and `db.MdocStore.GetBatchesByDocType` accessor, but an unused, unguarded lookup only waits for its next caller. Look batches up by format-specific accessor.
