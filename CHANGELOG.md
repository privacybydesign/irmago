...
## Unreleased
### Added
- The credential catalogue, schema 1.1 of the wallet configuration: an optional `credential_catalog` section, the OpenID4VC analogue of the IRMA scheme's credential store, keyed by credential type (vct). An entry carries an optional `vct_metadata_url` (which makes a URN vct resolvable), an opt-in `in_store` flag, and one or more offerings, each with per-language web issuance URLs (a `default` entry required, https only) and an optional `issuer_metadata_url`. Entries reference no trusted entity and confer no trust; names and attributes come from the type's and the issuer's metadata, the raw vct as the last resort. In the disclosure flow, a DCQL query with no owned candidates whose vct the catalogue lists now yields one obtainable descriptor per offering with `IssueURL` set for the current locale (exact tag, then base language, then default), which turns the permission prompt from a dead end into issuance during disclosure; a miss keeps today's behaviour. The offering's issuer is soft-matched against the trusted entities by the host it issues from (a certificate's subject alternative names, a did:web domain), and takes the entity's name and trust level when one matches. `Client.GetOpenID4VCCredentialStore` serves the browsable store's separate OpenID4VC section from the opted-in entries, one descriptor per offering; `GetCredentialStore` is untouched. Metadata is fetched on demand and cached while the same config issue is held. `yivi eudi config build` reads catalogue entries from `credentials/<name>/credential.json` in the curation directory, and `show` prints them; the built config now declares schema version 1.1.
- `yivi eudi config`: the publishing tools for the wallet configuration. `build` compiles a curation directory — `config.json` with the config's own information, one directory per trusted entity holding `entity.json` and the certificates it names by bare filename, the directory name being the entity id — into the payload the wallet reads, validated as the wallet validates it; curation files are read strictly, so a typo fails in review rather than publishing as a silently missing field, and a logo digest can be computed from a file beside the entity. `sign` signs the payload as the compact JWS the wallet fetches (ES256, `x5c` from `--cert`) and re-verifies it, against `--root` as the wallet will. `verify` re-checks a signed config with the wallet's own verifier, refuses one past its grace period, and with `--against <URL>` is the release gate: the candidate must be a later issue than the published config, or the wallet would refuse it as a rollback. `show` prints what a signed or unsigned config claims without verifying it, and `--json` prints the payload as `build` writes it, so the two can be diffed. `serve` serves a signed config over HTTP for development and staging. `keygen` writes a throwaway root, intermediate and signer chain for tests and pull-request checks. The example curation directory under `testdata/walletconfig/source/` builds the golden payload byte for byte.
- The wallet config is now the source of trust for the OpenID4VC side of the wallet. `eudi.Configuration.Reload` installs the `x509_ca` handles of the active environment's trusted entities as the issuer and verifier trust anchors, next to the chains installed in the eudi storage folder (the developer and test seam). The compiled-in PEM anchors moved out of `eudi/trustanchors.go` into `walletconfig.YiviEnvironments`, as built-in entities of the `production` and `staging` environments; until the Yivi Wallet Config CA exists both environments are unpublished and run on those alone, and the built-in entities are deleted at cutover. `client.New` now takes a `client.Config` struct (the named form of its former positional arguments) with two new fields: `Environments`, defaulting to the Yivi environments, and `AppBuild`, for the minimum app build gate. Developer mode is now the environment switch: it swaps the whole environment (anchors, listed parties, policy) to staging and back, where it used to add the staging anchors on top of the production ones. `InitJobs` takes a third interval for the wallet config refresh job; `Client.RefreshWalletConfig` runs the same refresh on demand, for the app to call when it comes to the foreground, throttled to once per `Config.WalletConfigRefreshInterval` (an hour by default) while the held config is fresh.
- `eudi/trust`: the trust ladder. `trust.Evidence` (the certificate or DID a party authenticated with) goes in, a `trust.Verdict` comes out: the effective level — the strongest across the matching entities, low when none matches — the matching entity, and whether the certificate chains to an anchor. An `x509_ca` handle matches a chain through its root and, when it lists intermediates, one of them, so two entities under one root are told apart; `x509_cert` and `did` handles match exactly. Once a config is expired its individual listings stop counting while its CA anchors keep working. `trust.CheckMinimum` applies the policy's minimum trust level per session type. A party under a locally installed anchor ranks high.
- Trust levels reach the app. `clientmodels.TrustedParty` gains `TrustLevel` and loses `Verified`: an IRMA issuer is always high, an IRMA verifier is high when it is in the requestor scheme and low otherwise, and an OpenID4VC party carries the level the ladder assigned it. The EUDI activity log stores the issuer's trust level per logged credential (`IssuerTrustLevel`; the old `IssuerVerified` column is no longer written and reads as unevaluated). The OpenID4VP requestor, the OpenID4VCI issuer, the offered credentials' issuers, the stored credentials' issuers and the disclosure candidates' issuers all carry it. `clientmodels.Credential` gains `IssuerNotTrusted`: the issuer falls short of the issuance minimum in the active environment, so the credential stays stored and visible but is not offered for disclosure. Three new session error types: `party_validation_failed` (the identity gate rejected the party: a broken, expired or revoked certificate, an invalid signature, an unresolvable DID), `trust_level_below_minimum` (the party's level is below the policy minimum for the session type) and `app_update_required` (the config's minimum app build is above this build).
- `eudi/walletconfig`: the wallet configuration, a signed document per environment that will carry the trust anchors and trust policy for the OpenID4VC side of the wallet, replacing the compiled-in PEM constants in `eudi/trustanchors.go` in a later step. This step is the document's lifecycle only. `walletconfig.Config` is the payload — schema version, environment, monotonic version, issue and next-update times with a grace period, a minimum app build, a policy of minimum trust levels per session type, and one `trusted_entities` list where each entity carries roles, a trust level and typed handles (`x509_ca`, `x509_cert`, `did`) plus optional constraints — and validates itself; fields, handle types and roles this client does not know are ignored, so a minor schema bump is not an app release. `walletconfig.Verify` checks a signed config against exactly one root, the one compiled in for its environment: `typ` `yivi-wallet-config+jwt`, `alg` ES256, an `x5c` chain to that root with the digitalSignature key usage on the leaf, and a payload declaring that environment. Nothing that chains to an issuer or verifier anchor can sign a config. `walletconfig.Manager` holds the current verified config for the active environment, loads it from the copy bundled with the release or the one persisted from an earlier fetch (whichever is newer, both verified like a download), refreshes it in the background, refuses any config that rolls back the held (version, issued_at), reports freshness as fresh, stale or expired against `next_update` and the grace period, throttles start/foreground refreshes to once an hour while fresh and fetches eagerly once stale, and switches environments atomically so anchors of two environments are never mixed. `Manager.Snapshot` never touches the network. Persisted configs live in the new `wallet_config_documents` table of the EUDI database (`db.NewWalletConfigStore`), filed under the config's `id`, which the environment descriptor names in `ConfigID` and `Verify` checks the payload against. `walletconfig.Sign` is the publisher's counterpart, taking a `crypto.Signer` so an HSM-held key signs the same as one in memory. A committed golden fixture under `testdata/walletconfig/golden/` pins the wire format. Not yet wired into the client: trust evaluation, the environment switch in developer mode, the CLI and the credential catalogue follow in later steps.
- `clientmodels.TrustLevel`: the three rungs of the trust ladder (`low`, `medium`, `high`) plus the unevaluated zero value, with `IsVouchedFor` and `IsRung`. Harvested from the `lote-trust-anchors` branch; the wallet config's `trust_level` fields use it.

### Changed
- **The uniform trust ladder replaces hard rejection of unanchored parties.** An OpenID4VP verifier whose certificate chains to no trust anchor used to fail the session; it is now admitted with trust level low, exactly like a bare `did:jwk` verifier, and the policy decides whether that is enough (the first policy is low/low, so nothing is blocked and the ladder drives warnings only). The identity gate keeps refusing a certificate that is expired, revoked, or does not match its `client_id`. The same holds for the issuer of an OpenID4VCI credential: the OpenID4VCI verification context sets the new `sdjwtvc.SdJwtVcVerificationContext.AcceptUnanchoredIssuers`; the verifier side and the SD-JWT-over-IRMA path leave it off and keep requiring an anchored issuer. DID verifiers and issuers are now anchored too, through the config's `did` handles; an unlisted DID is low.
- What an OpenID4VP verifier is shown as follows the trust ladder: the name and logo of the entity the wallet config lists for it first, what an anchored certificate's Yivi scheme extension says second, what the verifier says about itself last. A logo is only ever taken from a source beyond the verifier, so the `logo_uri` a verifier names for itself in `client_metadata` is no longer downloaded or shown, and `client_name` no longer outranks the name in an anchored certificate. The verifier's authorization is enforced with list-over-certificate precedence: a disclosure constraint the wallet config lists replaces the certificate's authorized attribute sets, the certificate's apply when it is anchored and nothing is listed, and an unanchored certificate authorizes nothing. On issuance, a listed issuance constraint refuses credential types outside it.
- `openid4vp.VerifierValidator.ParseAndVerifyAuthorizationRequest` returns an `openid4vp.VerifiedRequestor` (certificate or DID, the certificate's scheme data, the self-asserted name) instead of a certificate and scheme data; `NewRequestorCertificateStoreVerifierValidator` no longer takes a query validator factory. `openid4vp.NewClient` and `openid4vci.NewClient` take a `trust.Evaluator`; `services.NewCredentialService` and `eudi_sdjwt_dcql.NewSdJwtVcDcqlHandler` take the trust service. `eudi.Configuration` lost `SetUseStagingTrustAnchors` and `UsesStagingTrustAnchors`; its trust models now publish an immutable state per reload, so a session never sees the half-built middle of a reload. A trust anchor no longer has to be self-signed: a subordinate CA can be anchored on its own.
