...
## Unreleased
### Changed
- Update `github.com/privacybydesign/gabi` from the December 2022 pseudo-version `68a0869` to `1d483a2` (2026-07-30), 67 commits later. No gabi API changed, and nothing on the wire changed, so an updated irmago still issues to and discloses against older peers. The bump pulls in the `signed`, `gabikeys`, `revocation` and `credential` fixes listed under Fixed below. gabi's own `go` directive is 1.26.4, lower than irmago's 1.27, so it does not move irmago's minimum Go version. Carried along: `github.com/mr-tron/base58` 1.1.3 to 1.3.0 (used by `eudi/did`), `github.com/minio/sha256-simd` 0.1.1 to 1.0.1, `github.com/multiformats/go-multihash` 0.0.11 to 0.2.3, and the indirect deps that follow from those ([#711](https://github.com/privacybydesign/irmago/issues/711))
