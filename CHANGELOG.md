...
## Unreleased
### Added
- **The anchor list**: a second recognized list that delivers certificate authorities rather than parties, so Yivi can add, promote and withdraw a CA in the wallet without an app release. It is a TS 119 602 LoTE in the same Annex A JSON binding as the party list, typed `https://yivi.app/19602/LoTEType/YiviTrustAnchorsList`, whose services are CAs: `…/Svctype/IssuerCA` or `…/Svctype/VerifierCA` (role-typed, like the party types, because the wallet keeps one anchor pool per role), the CA certificate in `ServiceDigitalIdentity.X509Certificates`, the CRLs it issues in the binding's own `ServiceSupplyPoints`, and the level it confers in a `YiviConfers` extension (`medium` when silent). `lote.Source` gains `Delivers` — a source delivers parties (the default) or anchors, never both: a CA on a party list installs nothing and a party on an anchor list grants nobody — and `SignerSKI`, which pins the certificate the list must be signed with on top of the anchor set, so the party list's daily signing key cannot sign a document the wallet installs anchors from; an anchor source must set it, and `client.New` now refuses a misconfigured source set (`lote.ValidateSources`). `Source.Confers` on an anchor source is the ceiling an entry's level is capped by. The checker exposes granted CA services as `Anchors()`, `services.TrustService` installs them through `eudi.Configuration.SetListTrustAnchors` on every refresh and at start-up from the persisted list, and `RefreshTrustLists` reports a change — and wakes the app — when the anchor set changes. List-delivered anchors go into the issuer and verifier pools only: there is no way to hand one to the trust-list pool, so a list can never widen the set of keys allowed to sign lists. A pinned anchor a list also names keeps the stronger level, so a list can add and promote but never demote or remove what is compiled in. Everything about it is fail-soft like the party list: an unreachable, expired or unverifiable anchor list contributes no anchors and sessions run on the compiled-in ones.
- `yivi eudi lote build` reads anchor curation: `delivers: "anchors"` and `confers_ceiling` in `scheme.json`, and services with `role: issuer-ca` or `verifier-ca`, `identity.certificates`, `confers` and `crl_distribution_points` in `entity.json` (`testdata/lote-source-anchors/` is the worked example). It refuses what the wallet would otherwise skip with a log line: a CA role on a party scheme or a party role on an anchor scheme, a key or DID identity on a CA, a certificate that is not a CA or is outside its validity window, a level above the ceiling, `high` on a CA the operator does not run unless marked `contracted`, a missing or plaintext CRL distribution point, and the same CA under two entities. Unless `--offline`, it fetches every distribution point and requires the CRL to verify against the CA. `verify` gains `--signer-ski`, `show` renders anchor services, and `yivi eudi lote pinned [--json]` lists the anchors compiled into the binary as the wallet installs them, so a curation repository's CI can diff them against its own Yivi CA services. `jades.SignatureAlgorithmFor` decides on the public key, so an opaque `crypto.Signer` — an HSM or PKCS#11 token — signs the same as an in-memory key. A committed, signed golden anchor list lives in `testdata/lote-publisher/golden-anchors/`.

### Changed
- **Trust anchors are the CAs that issue end-entity certificates, not the roots above them.** The first certificate of an anchor chain is the anchor — the terminus of chain building, trusted for what it signs directly, whether or not it is self-signed — and any certificates after it are its issuers, kept only to check the anchor's own revocation. Nothing changes for the compiled-in Yivi anchors, which already list the Attestation Providers and Relying Parties CAs first: the same certificates validate at the same levels. What changes is that the shared Yivi root is no longer an anchor in any pool, so a certificate under a *sibling* CA beneath that root — say, an issuer certificate presented as a verifier — no longer validates anywhere; before, the pools were kept apart only by the wallet ignoring `x5c` intermediates. Anchor levels are keyed by the CA, an expired CA is skipped rather than its root anchored instead, and an issuer's expiry no longer disqualifies a valid CA under it. `TrustModel.Anchors()` lists what is installed.
...
- The recognized-list channel of the trust ladder: a party granted on a trust list the wallet recognizes now ranks what that list's *source* confers — `high` on Yivi's own LoTE, because being listed there is being onboarded and Yivi cannot name a party on its list without vouching for it, and at most `medium` on any other recognized list. A list is an ETSI TS 119 602 List of Trusted Entities in JSON, signed as a compact JAdES-B-B whose `x5c` chain validates against the pinned Yivi anchors, with role-typed entries — trust as an issuer is not trust as a verifier — keyed on certificate or subject key identifier plus the entity's `organizationIdentifier`, and on the bare DID for entries carrying one under the `other_ids` type `did`. Markings on entries are carried and ignored: the rung is the source's word, never the entry's. `Client.RefreshTrustLists(ctx) error` downloads the lists and adopts the ones that hold up; a list counts only while its signature verifies, it is before its `next_update`, and its sequence number has not gone backwards. Everything about the channel is fail-soft: an unreachable, tampered, expired or replayed list is absent evidence, so parties fall back to the certificate channel and sessions run on. The certificate and list channels are independent and a party lands on the higher rung, so a Yivi-certified verifier stays `high` with every list down ([#657](https://github.com/privacybydesign/irmago/issues/657)).
- `client.Config.RecognizedTrustLists`: the recognized lists a wallet consults, each with the trust level its entries confer, named by the call that builds it. A released wallet passes none until Yivi publishes its list, so it ranks parties by the certificate channel alone, exactly as before; an integration test or a staging build names the list it controls.
- Every pinned trust anchor now carries the trust level its certificates confer: the Yivi roots confer `high`, and a third-party CA would be anchored at `medium` — the blog's "somebody vouches, but not Yivi" — promotable to `high` under contract as a change to that data rather than to code. `client.Config.ExtraIssuerTrustAnchors` and `ExtraVerifierTrustAnchors` pin extra anchors with their level. Classification of a *stored* credential's issuer certificate is expiry-tolerant — the chain is verified at a time inside the leaf's own validity window, because issuer certificates routinely expire before the credentials they signed — while revocation and a dropped anchor still demote; a certificate presented live outside its validity window keeps failing the session.
- The certificate channel of the trust ladder now reaches OpenID4VCI issuers: an issuer that signs its credentials with an `x5c` chain validating against the wallet's trust anchors ranks the level that anchor confers — `high` under the Yivi roots, the same rung a certificate-authenticated verifier reaches. `sdjwtvc.VerifiedSdJwtVc` carries the end-entity certificate (`IssuerCertificate`, nil for a DID-identified issuer) that the holder verification path previously validated and dropped. Each offered credential is ranked separately, off the certificate and `iss` it was actually signed with, alongside the session's own issuer rung — and both are ranked after the credentials are fetched, so the permission screen and the issuance log show what was verified rather than what the issuer's metadata claimed at offer time. A `low` issuer still issues: the rung is rendered, never enforced ([#660](https://github.com/privacybydesign/irmago/issues/660)).
- A stored credential's issuer rung now follows the recognized lists. Issuance records the issuer's evidence on the batch — the certificate it signed with, next to the identifier it named — and every credential read ranks the issuer against the list state of that moment, so an issuer delisted after issuance demotes on the next read and a newly listed one promotes. No level is stored and nothing is migrated: batches written before this carry no certificate and rank through the list channel alone, exactly as a DID-identified issuer does. Both credential-reading paths do this, the credential list and the disclosure plan, so a credential cannot show one rung in the wallet and another in a session. Activity log levels are untouched — they are session-time snapshots — and a withdrawn issuer stays distinct from a revoked credential (`Issuer.TrustLevel` vs `Revoked`) ([#661](https://github.com/privacybydesign/irmago/issues/661)).
...
### Changed
- **Breaking:** `clientmodels.TrustedParty` carries a `TrustLevel` (`"low"`, `"medium"`, `"high"`) instead of the boolean `Verified`, and the app must update in lockstep: derive whatever it showed for `verified` from `trust_level` being medium or high. The level says how strongly a party is vouched for, never whether it is who it claims to be: identity validation is a separate gate that fails the session, and a low level never blocks one. The field is absent when nothing was evaluated, which is not the same as low. What the wallet reports today: an EUDI verifier whose certificate chain validates against the Yivi trust anchors and an IRMA issuer or requestor registered in a valid scheme rank high; a party on Yivi's trust list ranks high and a party under an anchored third-party CA would rank medium, though neither is reachable in production until the list is published and such a CA is anchored; everyone else ranks low ([#655](https://github.com/privacybydesign/irmago/issues/655)).
- An X.509 party whose chain the wallet cannot trace to any anchor no longer fails the session: a chain that ends nowhere the wallet knows proves nothing, so its holder passes the identity gate as a legitimate-looking stranger and ranks `low`, with the certificate's contents — name included — treated as the party's own word. An entry on a recognized trust list keyed on that very certificate still lifts the party, all the way to `high` on Yivi's own list. A revoked certificate is not this case: revocation is the CA withdrawing a certificate it issued, and it is how a compromised issuer or relying party is cut off, so it stays a refusal on both EUDI paths — the disclosure request fails with `party_validation_failed`, and a credential signed with a revoked issuer certificate is not stored. The gate keeps rejecting genuinely broken requests too — a bad signature, a client_id the certificate does not cover, a certificate outside its own validity window.
- **Breaking:** what an EUDI party is called and pictured now resolves by how strongly it is curated, through one helper both protocol paths compose through: the name on the recognized-list entry first, the name an *anchored* certificate attests second, and the name the party gives itself last. The verifier validator hands the attested and self-asserted accounts over separately, so a `client_metadata` name never rides along as attested just because a certificate was also presented — it is the verifier's own word, shown only when nothing curated or attested names the party, and its `logo_uri` is not even downloaded. A logo only ever comes from a source beyond the party — the list entry or an anchored certificate — so a party nobody vouches for is shown with its own name, its identifier, and no logo. For the app this means the issuer party on an OpenID4VCI permission request now arrives without an `Image`, and a Yivi-certified verifier that also sends `client_metadata` is now shown under its certificate's attested name rather than the metadata's. The logo on a *credential's* issuer, which the credential list and the activity log render, is unchanged. Curated logos are downloaded on first sight and cached like any other, keyed by their URI ([#659](https://github.com/privacybydesign/irmago/issues/659)).
- **Breaking:** `client.New` takes a single `client.Config` instead of eight positional arguments, and is the wallet's only constructor. Fields are named because three of them are plain string paths (`StoragePath`, `IrmaConfigurationPath`, `EudiAppDataPath`) that a caller could transpose without the compiler noticing; the zero value of a field takes a documented default, so `Locale: ""` is the fallback language and `RecognizedTrustLists: nil` consults no trust lists. It also replaces `NewWithRecognizedTrustLists`, which existed only to vary that last field.
- **Breaking:** `Client.InitJobs` takes a third interval, for the recognized trust list refresh; pass 0 to leave the job off, as the other intervals do. The job calls `RefreshTrustLists`, which now signals `ClientHandler.CredentialsChanged` when a list comes back saying something different about the parties on it — a stored credential's issuer rung is read fresh on every listing, so what the app is showing went stale. A re-issue carrying the same entries is silent, the same way a re-confirmed credential status is ([#661](https://github.com/privacybydesign/irmago/issues/661)).
- EUDI activity log entries record the trust level the session showed: the requestor's on the entry (the verifier on a disclosure, the issuer on an issuance) and each credential's issuer's on the credential. The levels are a snapshot, written once and never re-resolved, so a party vouched for or delisted later does not rewrite history — unlike the display text above, which does follow live metadata. Both columns are nullable and auto-migrated: rows written before this version carry no requestor level and render levelless, and their issuer level is read off the `IssuerVerified` boolean it replaces, which only ever recorded "Yivi vouches" and so maps to high or to nothing at all. Absent stays distinct from a verdict of `low`, and nothing writes the boolean any more ([#658](https://github.com/privacybydesign/irmago/issues/658)).
...
### Fixed
- A verifier holding a Yivi certificate could skip the attribute authorization that certificate carries by sending `client_metadata` with its request: the query validation only ran when the wallet fell back to the certificate for display. The authorization is now enforced whenever an anchored certificate carries one, whatever else the request contains — a request exceeding it fails at any trust level.
- An OpenID4VCI issuer granted on a recognized trust list is now shown under the **curated name and logo** the list entry carries, not only at the curated trust level. The party was composed once at offer time, when the only evidence the wallet holds is the issuer's own identifier, and the post-fetch re-ranking updated the rung while leaving the display untouched — so an issuer a list names by the DID it signs with (the common case, since a credential's `iss` is not the `credential_issuer` URL) reached the permission screen at `medium` under the name it had given itself, with the curation silently dropped. The party is now composed again once the credentials are in hand, from the same verdict that sets the rung.
...
### Internal
- `eudi.TrustModel` now holds its anchors, intermediates, revocation lists and anchor levels as one immutable state that a reload assembles off to the side and publishes whole; `Configuration.Reload` publishes all three models together, only once every step has succeeded, and is serialised. Before, a reload emptied the model first and refilled it step by step, so a session running meanwhile — or every session until the next CRL tick, if the reload failed on an unparseable extra anchor — saw no revocation lists at all. On `master` that window failed closed, because the roots were gone too; with unanchored certificates now admitted as `low`, it had become fail-open: a revoked certificate passed the identity gate. A model that has not reloaded yet also verifies against an empty root pool rather than a nil one, which `x509.Verify` would have read as the operating system's root store. `TestTrustModel_ReadersNeverSeeAHalfBuiltReload` classifies a revoked leaf while reloads run and requires it refused every time.
- `yivi eudi lote sign` reads its input strictly and validates it against the Annex A schema before signing. It used to decode leniently and sign whatever `lote.Sign` re-marshalled, so a hand-edited document with a mandatory member deleted was signed with that member rendered as `null`, an unknown member was silently dropped, and `verify` — which, like the wallet, does not schema-check — passed the result. `build` already validated what it wrote; `sign` is a standalone command that takes any file, so it is now a gate of its own.
- End-to-end integration tests for the recognized-list channel against a **real published list**: a `lote_publisher` compose service (`testdata/lote-publisher`) serving an ETSI TS 119 602 list over TLS through the existing nginx proxy, signed with the **openssl CLI** rather than the JWS library the wallet verifies with — so the suite proves interop with a foreign toolchain instead of testing a library against itself. Six scenarios in `TestSessionHandler/eudi/published-trust-list`: the tracer (a foreign-signed list grants `medium`), a held list crossing `next_update` mid-run, the scheduled refresh waking the app exactly once, a session completing while the publisher is dark, a restarted wallet ranking from the persisted document, and curated name + logo beating the ones the verifier's own certificate carries. Certificate keying (`x509_ski` + `organizationIdentifier`) is covered on both roles, with the negative cases in-process. No new wallet seams were needed; the expiry scenario costs ~7s by publishing `next_update` just inside `lote.ClockSkew` rather than waiting the window out. Specified in [`docs/plans/lote-e2e-tests.md`](docs/plans/lote-e2e-tests.md).
- A **committed, signed golden LoTE** (`testdata/lote-publisher/golden/`) with `eudi/trust/lote/golden_test.go` reading it. Every other test builds its list out of the Go structs in `model.go`, so a change to their JSON tags — renaming `next_update`, dropping `other_ids` — passes the whole suite while breaking every list a real publisher emits; those tags are the wire contract, and this is the only test that can notice them changing. It doubles as documentation: `golden/list.json` is the same list in readable form, and a test proves the two parse identically, so the readable copy cannot drift. Between them they show every shape the wallet understands — both certificate key forms, a DID, the `onboarded-by-yivi` marking, a withdrawal, service-level name and logo overrides, an unknown marking carried but not acted on, multilingual names. It cannot rot: `verify` deliberately ignores the list's own time bounds, and the currency and keying assertions pin a clock derived from the signing certificate rather than reading the wall clock. Regenerate with `mkgolden.sh`, which signs through the publisher's own code path.
- The EUDI test verifier's signing leaf (`testdata/eudi/verifier`) is regenerated to carry an `organizationIdentifier` (so trust-list entries can be keyed on the certificate *and* the legal entity) and to expire in 2036 rather than 2028. The key is reused, so its subject key identifier is unchanged — which is also the "an SKI-keyed entry survives a renewal" property the format is keyed that way for.
- `eudi/trust/lote` ships its test infrastructure in a non-test file, like the statuslist package: a two-level test signing chain and a mutable httptest LoTE server, so session tests in other packages can publish a list they control and change it mid-test.
- Integration tests for the trust ladder in `internal/sessiontest/trust_levels_test.go`: both roles at their rungs as real sessions, the fail-soft floor, the role-typed grant, and an over-ask against a certificate-carried authorization failing at the top rung. Its header documents which compose counterparty reaches which rung through which channel — the veramo issuer and verifier by DID, the Python PID issuer and the EUDI Kotlin verifier by certificate — so a test needing a new rung adds a list entry rather than a counterparty, and records the five mutation checks that guard the ladder's decisions, each verified by disabling the guard and watching the named test fail ([#662](https://github.com/privacybydesign/irmago/issues/662)).
- Storage regression snapshot for v1.3.0 (`testdata/storage_regression/v1.3.0/`), verified by `TestClientStorageRegressionV1_3_0`. This is the regeneration v1.3.0 called for: it is the first snapshot whose activity logs store text already resolved to a locale, rather than the translation maps the v1.0.0 and v1.1.1 snapshots hold, and the first whose OpenID4VCI issuance logs record the issued JWT's `vct` as the credential id instead of a placeholder. The older snapshots stay as they are, so the legacy translation-map decode path keeps its regression coverage.
- The regression fixture generator now also issues a status-list credential and revokes it, so the snapshot carries Token Status List state at rest — instances with a `status.status_list` reference and the INVALID `LastKnownStatus` the revocation left behind — which no earlier snapshot has. `TestClientStorageRegressionV1_3_0` asserts that a reloaded client reports the credential as revoked and revocation-supporting while a credential without a status reference reports neither. Both flags come from stored status with no network access, so the assertions do not depend on the status-list agent still serving that list and index.
