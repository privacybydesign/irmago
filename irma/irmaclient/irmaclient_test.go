...
func parseStorage(t *testing.T) (*clientstorage.Storage, *IrmaClient, *TestClientHandler) {
	storage := test.SetupTestStorage(t)
	return parseExistingStorage(t, storage)
}

func parseExistingStorage(t *testing.T, storageFolder string) (*clientstorage.Storage, *IrmaClient, *TestClientHandler) {
...
func TestFreshStorage(t *testing.T) {
	s := test.CreateTestStorage(t)
	storage, client, _ := parseExistingStorage(t, s)
	defer storage.Close()
	defer client.Close()
