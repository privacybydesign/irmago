...
func (cr *CredentialRequest) Validate(conf *Configuration) error {
	...
	// Note: the random blind attribute identifiers sent by the server are checked in
	// IssuanceRequest.checkRandomBlindConsistency, which has access to the negotiated protocol
	// version needed to interpret them (see RandomBlindAttributeNames[Legacy]).
...
func (ir *IssuanceRequest) GetCredentialInfoList(
	...
	if err := ir.checkRandomBlindConsistency(conf, version); err != nil {
		return nil, err
	}
...
// interoperable with an unpatched server and vice versa.
