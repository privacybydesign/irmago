...
// Deleting a credential whose type is gone from the configuration — the very
...
func TestCreateRemovalLog_UnknownTypeLogsWithoutAttributes(t *testing.T) {
	...
	entry, err := createRemovalLog(conf, id,
		...
		[]clientmodels.CredentialFormat{clientmodels.Format_Idemix})

	require.NoError(t, err)
	require.Contains(t, entry.Removed, id)
	assert.Empty(t, entry.Removed[id])
	assert.Equal(t, []clientmodels.CredentialFormat{clientmodels.Format_Idemix}, entry.RemovedFormats)
}

// The activity log must render the removal of a credential whose type is no
// longer in the configuration: the entry falls back to the bare identifiers
// instead of dereferencing the missing type and taking the whole log down.
func TestRawLogEntryToLogInfo_RemovalOfUnknownTypeRenders(t *testing.T) {
	...
	entry := &irmaclient.LogEntry{
		...
		RemovedFormats: []clientmodels.CredentialFormat{clientmodels.Format_Idemix},
	}
	...
	assert.Equal(t, []clientmodels.CredentialFormat{clientmodels.Format_Idemix}, cred.Formats)
}
