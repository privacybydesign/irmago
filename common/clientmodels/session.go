...
// ErrorType_PartyValidationFailed is the SessionError.ErrorType a session ends
// with when the identity gate rejected the party it was talking to: a broken or
// revoked certificate chain, an invalid request signature, an unresolvable DID.
//
// It exists so the app can tell this apart from a network or protocol hiccup
// and show the blocking copy ("the request is not trustworthy", "no data has
// been shared") instead of a generic error screen. A party that merely fails to
// be vouched for is not this — that is a low trust level, and the session
// proceeds (see TrustLevel).
