...
//
// It exists so the app can show the blocking copy ("the request is not
// trustworthy, no data has been shared") rather than a generic error screen. A
// party that merely fails to be vouched for is not this — that is a low trust
// level, and the session proceeds (see TrustLevel).
