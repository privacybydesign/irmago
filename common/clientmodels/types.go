...
type CredentialDescriptor struct {
	...
	// The credential type's FAQ texts from the scheme, resolved to the
	// current locale, so frontends can show the explanation content in flows
	// that only receive a descriptor (e.g. obtaining a missing credential
	// during disclosure). Nil for credentials without a scheme entry, such as
	// generic EUDI credentials.
