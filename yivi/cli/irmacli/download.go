...
var downloadCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if path == defaultIrmaconf {
			if defaultIrmaconf == "" {
				clihelpers.Die("Failed to determine default irma_configuration path", nil, Logger)
			}
			if err := common.EnsureDirectoryExists(defaultIrmaconf); err != nil {
				clihelpers.Die("Failed to create irma_configuration directory", err, Logger)
			}
			fmt.Println("No irma_configuration path specified, using " + defaultIrmaconf)
		}
		if err := downloadSchemeManager(path, urls); err != nil {
			clihelpers.Die("Downloading scheme failed", err, Logger)
		}
	},
}
