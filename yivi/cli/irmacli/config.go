	...
	if confpath != "" {
		...
		if err != nil {
			if os.IsNotExist(err) {
				clihelpers.Die("specified configuration file does not exist", nil, Logger)
			} else {
				clihelpers.Die("failed to stat configuration file", err, Logger)
			}
		}
		if info.IsDir() {
			clihelpers.Die("specified configuration file is a directory", nil, Logger)
		}
