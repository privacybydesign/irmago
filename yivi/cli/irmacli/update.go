...
var updateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) != 0 {
			...
			if irmaconf == "" {
				clihelpers.Die("Failed to find default irma_configuration path", nil, Logger)
			}
			files, err := os.ReadDir(irmaconf)
			if err != nil {
				clihelpers.Die("Failed to read default irma_configuration path", err, Logger)
			}
		...
		if err := updateSchemeManager(paths); err != nil {
			clihelpers.Die("Updating schemes failed", err, Logger)
		}
	},
}
