...
100% most of the time. Up to 8 GB RAM may be used.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			clihelpers.Die("Error reading public key", err, Logger)
		}
		...
		if err != nil {
			follower.StepDone()
			clihelpers.Die("Error opening proof", err, Logger)
		}
		defer closeCloser(proofFile)
		proofGzip, err := gzip.NewReader(proofFile)
		if err != nil {
			follower.StepDone()
			clihelpers.Die("Error reading proof data", err, Logger)
		}
		...
		if !s.VerifyProof(proof) {
			clihelpers.Die("Proof is invalid!", nil, Logger)
		} else {
