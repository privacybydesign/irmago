	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if flags.Changed("static-server") {
			if err := staticRequest(flags); err != nil {
				clihelpers.Die("Failed to handle static session", err, Logger)
			}
		...
		if url != defaulturl && serverURL != "" {
			clihelpers.Die("Failed to read configuration", errors.New("--url can't be combined with --server"), Logger)
		}
		...
		if jsonPkg == "" {
			...
			if serverURL != "" {
				...
				if err != nil {
					clihelpers.Die("Session could not be started", err, Logger)
				}
			}
		} else {
			pkg = &server.SessionPackage{}
			...
			if err != nil {
				clihelpers.Die("Failed to parse session package", err, Logger)
			}
		}
		...
		if err != nil {
			clihelpers.Die("Session failed", err, Logger)
		}
...
func staticRequest(flags *pflag.FlagSet) error {
	...
	flags.Visit(func(f *pflag.Flag) {
		...
		default:
			clihelpers.Die("Invalid configuration", errors.New("When using --static-server, only --static-name and --noqr can be used"), Logger)
		}
	})

	if name == "" {
		clihelpers.Die("Failed to read configuration", errors.New("--static-server must be combined with --static-name"), Logger)
	}
...
func init() {
	IrmaRootCmd.AddCommand(sessionCmd)
