...
may be used.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err = common.AssertPathExists(path); err != nil {
			clihelpers.Die("Nonexisting path specified", err, Logger)
		}
		...
		if err != nil {
			clihelpers.Die("Could not read public key", err, Logger)
		}

		// Try to read private key
		sk, err := gabikeys.NewPrivateKeyFromFile(privkeyfile, false)
		if err != nil {
			clihelpers.Die("Could not read private key", err, Logger)
		}

		// Validate that they match
		if pk.N.Cmp(new(big.Int).Mul(sk.P, sk.Q)) != 0 {
			clihelpers.Die("Private and public key do not match", nil, Logger)
		}

		// Validate that the key is eligble to proving
		if !keyproof.CanProve(sk.PPrime, sk.QPrime) {
			clihelpers.Die("Private key not eligible to proving", nil, Logger)
		}
		...
		if prooffile == "" {
			...
			if err = common.EnsureDirectoryExists(proofpath); err != nil {
				clihelpers.Die("Failed to create"+proofpath, err, Logger)
			}
		...
		if err != nil {
			clihelpers.Die("Error opening proof file for writing", err, Logger)
		}
		...
		follower.StepDone()
		if err != nil {
			clihelpers.Die("Could not write proof", err, Logger)
		}
	},
}
