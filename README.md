...
If you use a managed Redis service from a cloud provider, please be aware that if you enable high availibility or cluster mode, they use Redis Cluster or Redis Sentinel under water. The `irma server` does not automatically detect this, so it does not require you to set the `--redis-accept-inconsistency-risk` flag. However, the same inconsistency risks holds here too.
