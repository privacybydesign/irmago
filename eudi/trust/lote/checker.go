...
func (c *Checker) Refresh(ctx context.Context) error {
	...
	var failures []error
	for _, source := range c.sources {
		if err := c.refreshSource(ctx, source); err != nil {
...
func (c *Checker) refreshSource(ctx context.Context, source Source) error {
	...
	if got := verified.list.SchemeInformation.ListIdentifier; got != source.ListId {
		return fmt.Errorf("declares list identifier %q, expected %q", got, source.ListId)
	}

	if !verified.current(c.now()) {
		return fmt.Errorf("expired: next_update was %s", verified.list.SchemeInformation.NextUpdate.Format(time.RFC3339))
	}
	...
	if previous, ok := c.held[source.ListId]; ok {
		if verified.list.SchemeInformation.SequenceNumber < previous.SchemeInformation.SequenceNumber {
			return fmt.Errorf("sequence number regressed from %d to %d",
