...
// that does not build a chain can still refuse a certificate its CA withdrew.
...
// CheckCertificateValidAt reports whether cert is inside its own validity window
// at the context's verification time, allowing skew on either bound. what names
// the certificate in the error message.
//
// A certificate presented outside its window is a broken artifact, so a gate that
// meets a live party refuses it whether or not the chain anchors anywhere.
...
// VerifyCertificateChains is the certificate acceptance policy in one place: the
// chain must build to an anchor of the context, the end-entity certificate must
...
//
// If a hostname is provided, it is used for the SAN check. A chain that reaches
// no anchor fails with an x509.UnknownAuthorityError inside the returned error,
// which callers ranking rather than gating tell apart with errors.As.
func VerifyCertificateChains(context X509VerificationContext, cert *x509.Certificate, hostname *string) ([][]*x509.Certificate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to verify x5c end-entity certificate: %w", err)
	}
	...
	if err := CheckCertificateNotRevoked(context, cert); err != nil {
		return nil, fmt.Errorf("failed to verify x5c end-entity certificate against revocation lists: %w", err)
	}
...
func VerifyCertificate(context X509VerificationContext, cert *x509.Certificate, hostname *string) error {
	_, err := VerifyCertificateChains(context, cert, hostname)
	return err
}
