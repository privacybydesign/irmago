...
func TestDownloadRemoteImage_DataURI(t *testing.T) {
	...
	t.Run("base64 encoded image", func(t *testing.T) {
		...
		data, mediaType, err := client.downloadRemoteImage(metadata.RemoteImage{Uri: uri})
		require.NoError(t, err)
	...
	t.Run("invalid data URI without comma", func(t *testing.T) {
		...
		_, _, err := client.downloadRemoteImage(metadata.RemoteImage{Uri: uri})
		require.Error(t, err)
	...
	t.Run("HTTP URI still fetched from server", func(t *testing.T) {
		...
		data, mediaType, err := client.downloadRemoteImage(metadata.RemoteImage{Uri: server.URL + "/logo.png"})
		require.NoError(t, err)
		require.Equal(t, "image/png", mediaType)
