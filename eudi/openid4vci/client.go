...
// mergeCredentialMetadata returns a merged view where VCT-derived fields win
// where present and the issuer's credential_metadata fills in gaps. Per
// OID4VCI v1.0 §12.2.4 the SD-JWT VC type metadata is "always preferred"; the
// wallet treats that as field-level preference so a minimal VCT document
// doesn't blank out the issuer's richer credential_metadata.
//
// Display: per-locale union. VCT entries win for locales they carry;
// credential_metadata entries fill in locales the VCT omits.
// Claims: per-path union. For paths in both, the VCT claim's Display
// supersedes credential_metadata's when non-empty, otherwise the credential_metadata
// Display survives. Mandatory is taken from VCT when set. Claim ordering
// follows credMeta first, then VCT-only paths in their original order.
func mergeCredentialMetadata(credMeta, vctMeta *metadata.CredentialMetadata) *metadata.CredentialMetadata {
	if vctMeta == nil {
		return credMeta
	}
	if credMeta == nil {
	...
	out := *vctMeta
	out.Display = mergeCredentialDisplays(credMeta.Display, vctMeta.Display)

	vctByPath := make(map[string]metadata.ClaimsDescription, len(vctMeta.Claims))
	for _, c := range vctMeta.Claims {
		vctByPath[claimPathKey(c.Path)] = c
	}
	credPaths := make(map[string]struct{}, len(credMeta.Claims))
	merged := make([]metadata.ClaimsDescription, 0, len(credMeta.Claims)+len(vctMeta.Claims))
	for _, c := range credMeta.Claims {
		key := claimPathKey(c.Path)
		credPaths[key] = struct{}{}
		if vctClaim, ok := vctByPath[key]; ok {
			c.Display = mergeClaimDisplays(c.Display, vctClaim.Display)
			if vctClaim.Mandatory != nil {
				c.Mandatory = vctClaim.Mandatory
			}
		}
		merged = append(merged, c)
	}
	for _, c := range vctMeta.Claims {
		if _, seen := credPaths[claimPathKey(c.Path)]; seen {
			continue
		}
		merged = append(merged, c)
	}
...
// mergeCredentialDisplays unions two display lists by locale: each VCT entry
// overrides the credential_metadata entry with the same locale (and entries
// with empty locale collide into one slot), and remaining credential_metadata
// locales survive.
func mergeCredentialDisplays(credDisplays, vctDisplays metadata.CredentialDisplays) metadata.CredentialDisplays {
	if len(vctDisplays) == 0 {
		return credDisplays
	}
	if len(credDisplays) == 0 {
		return vctDisplays
	}
	vctByLocale := make(map[string]struct{}, len(vctDisplays))
	for _, d := range vctDisplays {
		vctByLocale[displayLocaleKey(d.Locale)] = struct{}{}
	}
	out := make(metadata.CredentialDisplays, 0, len(credDisplays)+len(vctDisplays))
	out = append(out, vctDisplays...)
	for _, d := range credDisplays {
		if _, seen := vctByLocale[displayLocaleKey(d.Locale)]; seen {
			continue
		}
		out = append(out, d)
	}
...
// mergeClaimDisplays unions two []Display lists by locale, with VCT entries
// winning for locales they provide.
func mergeClaimDisplays(credDisplays, vctDisplays []metadata.Display) []metadata.Display {
	if len(vctDisplays) == 0 {
		return credDisplays
	}
	if len(credDisplays) == 0 {
	...
	out := make([]metadata.Display, 0, len(credDisplays)+len(vctDisplays))
	out = append(out, vctDisplays...)
	for _, d := range credDisplays {
		if _, seen := vctByLocale[displayLocaleKey(d.Locale)]; seen {
			continue
...
func displayLocaleKey(locale *string) string {
	if locale == nil {
		return ""
	}
	return *locale
...
func claimPathKey(path metadata.ClaimsPathPointer) string {
