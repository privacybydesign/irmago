...
type CredentialBatch struct {
	...
	// IssuerMetadataID is the FK to the IssuerMetadata record that was current at issuance time.
	// Nil if the issuer metadata was not available when the credential was stored.
	IssuerMetadataID *uuid.UUID      `gorm:"type:uuid;index"                                                      json:"issuer_metadata_id,omitempty"`
	IssuerMetadata   *IssuerMetadata `gorm:"foreignKey:IssuerMetadataID;references:ID;constraint:OnDelete:SET NULL" json:"issuer_metadata,omitempty"`
