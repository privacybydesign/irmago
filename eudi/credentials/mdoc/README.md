...
| BLE *radio* | ✗ | the only piece left, and it is native rather than Go: irmamobile scans the engagement UUID, subscribes to State/Server2Client and pumps bytes. Everything above it, up to and including the app-facing entry point, is complete and exercised. What the native layer has to build — and what it must not — is specified in [`eudi/proximity/INTEGRATION.md`](../../proximity/INTEGRATION.md) |
...
scanning, connecting and moving bytes over GATT, in irmamobile — and is specified in
[`eudi/proximity/INTEGRATION.md`](../../proximity/INTEGRATION.md), down to the
characteristic UUIDs, the MTU arithmetic and the one threading rule that deadlocks
the app if it is ignored.
