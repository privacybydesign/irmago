...
func TestEMacKeyUsesTheStaticDeviceKey(t *testing.T) {
	...
	ephemeral, err := GenerateEDeviceKey()
	if err != nil {
...
// TestEMacKeyInfoStringDiffersFromSessionKeys: EMacKey shares its salt with
// SKReader and SKDevice and is separated from them only by the info string, so a
// copy-paste of the wrong one is a live risk.
func TestEMacKeyInfoStringDiffersFromSessionKeys(t *testing.T) {
	f := newMacFixture(t)
	...
	sessionKeys, err := sessionKeysFromZAB(zab, f.transcript)
	if err != nil {
		t.Fatalf("sessionKeysFromZAB: %v", err)
	}
	if bytes.Equal(emacKey, sessionKeys.SKReader) || bytes.Equal(emacKey, sessionKeys.SKDevice) {
		t.Fatal("EMacKey equals a session key: the HKDF info string is not being applied")
	}

	// And it is exactly the derivation the clause states, computed independently.
