...
func NewVPTokenJSON(queryId string, resp mdoc.DeviceResponse) (string, error) {
...
// DeviceResponse.
func ParseVPTokenJSON(vpToken, queryId string) (mdoc.DeviceResponse, error) {
	var content map[string][]string
	if err := json.Unmarshal([]byte(vpToken), &content); err != nil {
		return mdoc.DeviceResponse{}, fmt.Errorf("decode vp_token: %w", err)
	}
	creds, ok := content[queryId]
	if !ok || len(creds) == 0 {
		return mdoc.DeviceResponse{}, fmt.Errorf("vp_token has no credential for query id %q", queryId)
	}
	encoded, err := base64.RawURLEncoding.DecodeString(creds[0])
	if err != nil {
		return mdoc.DeviceResponse{}, fmt.Errorf("base64url-decode credential: %w", err)
	}
