...
func TestResponseStatusCodesAreNotSessionStatusCodes(t *testing.T) {
	...
	// failure; Table 8's 10 is the mdoc declining to answer.
	//
	// Table 20's constants belong to the session layer, which this branch does
	// not carry, so the collision is pinned against the literals the clause
	// gives rather than against those names. The guard is the same one either
	// way: whoever reintroduces Table 20 must not fold it into these constants.
	const (
		table20SessionEncryptionError = 10
		table20SessionTermination     = 20
	)
	if ResponseStatusGeneralError != table20SessionEncryptionError {
		t.Fatal("test premise changed: Table 8 and Table 20 no longer collide on 10")
	}
	if ResponseStatusCBORValidationError == table20SessionTermination {
