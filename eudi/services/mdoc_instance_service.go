...
// # Why package scope
//
// A selector is built per session: each transport's handler constructs its own
// over a fresh store — mdoc_dcql.NewMdocDcqlHandler today, the ISO 18013-5
// transports as they land. State
...
func RevealFromClaimPaths(claimPaths [][]any) (map[string][]string, error) {
	...
	// loop below, which would hand SelectiveDiscloseNamespaces an empty reveal
	// map and leave IssuerSigned.NameSpaces an empty map with no error. ISO/IEC
	// 18013-5 has `IssuerNameSpaces = {+ NameSpace => [+ IssuerSignedItemBytes]}`
	// — one or more — so that document is not merely empty, it is
	// non-conformant, and it would be signed and posted with nothing naming the
	// defect.
	//
	// Not unreachable via a query check upstream: this takes the paths from the
	// user's selection, not from the query, and the OpenID4VP path drops
	// zero-length paths before building it — which turns a selection of one
	// empty path, an error below, into an empty slice that would otherwise pass
	// silently. The guard lives here rather than in either caller because both
	// transports strip documents through this function, and a rule enforced on
	// one of them only is the drift this package exists to prevent.
	if len(claimPaths) == 0 {
